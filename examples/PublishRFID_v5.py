...
while True:
    ...
    if id != previous_id:
        for x in SCUTTLE_IDs:
            #Make the Channel_Number equal to the last 4 digits of the ID number
            Channel_Number = abs(x) % 10000
            print("Channel Number: %s" % Channel_Number)
            client.virtualWrite(Channel_Number, SCUTTLE_STATUS[n], dataType="prox", dataUnit="d")
            n=n+1
    print(SCUTTLE_IDs)
    print(SCUTTLE_STATUS)
    sleep(1.5)
